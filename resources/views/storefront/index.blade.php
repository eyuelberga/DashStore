<x-store-layout :store="$store">
    ...
    <div class="container mx-auto">
        ...
        <div class="flex items-center flex-wrap pt-4 pb-12">
            ...
            @foreach ($store->products()->orderBy('created_at', 'DESC')->paginate(5) as $product)
                <x-product-card :link="route('storefront.product', ['store' => $store, 'product' => $product])" :image="$product->image" :name="$product->name" :qty="$product->qty"
                    :price="$product->price">
        ...
        <div class="flex justify-center flex-col p-6">
            {!! $store->products()->orderBy('created_at', 'DESC')->paginate(5)->links() !!}
            <div>
